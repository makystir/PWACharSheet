...
export function AdvancementPage({ character, update, updateCharacter }: AdvancementPageProps) {
  ...
  // can be used as a dependency of the sortedSkills memo below without
  // recreating it every render. The React Compiler can't statically prove
  // careerLevel stable (it's derived from allLevels.find), so it emits a
  // preserve-manual-memoization advisory — the memo is nonetheless correct
  // (recomputes exactly when careerLevel changes), so the advisory is suppressed.
  // eslint-disable-next-line react-hooks/preserve-manual-memoization
  const careerSkills = useMemo(() => careerLevel?.skills ?? [], [careerLevel]);
