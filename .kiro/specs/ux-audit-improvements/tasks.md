...
- [ ] 1. Extend the data model for combat target and initiative formula
  - [ ] 1.1 Add `combatState.target` and `houseRules.initiativeFormula`
...
- [ ] 2. Implement pure combat-target logic in `src/logic/combat-target.ts`
  - [ ] 2.1 Implement target get/set/clear helpers
  ...
  - [ ] 2.2 Write property test for combat-target helpers
...
- [ ] 3. Implement initiative rolling in `src/logic/initiative.ts`
  - [ ] 3.1 Add `rollInitiative` and `InitiativeRollResult`
  ...
  - [ ] 3.2 Write property test for initiative rolling
...
- [ ] 4. Checkpoint - core combat/initiative logic
  - Run the new logic tests and type-check; ensure all pass; ask the user if questions arise.

- [ ] 5. Wire the Combat_Target into `AttackFlow.tsx` (Tier 1, Req 1, 2)
  - [ ] 5.1 Read/write opponent TB/AP from the Combat_Target
  ...
  - [ ] 5.2 Add the net-wounds breakdown tooltip
  ...
  - [ ] 5.3 Log the attack result to the event log (dep: unified-event-log)
  ...
  - [ ] 5.4 Write render tests for AttackFlow target + logging
...
- [ ] 6. Reduce Take-Damage friction in `TakeDamagePanel.tsx` (Tier 1, Req 3)
  - [ ] 6.1 Add the Advanced_Qualities_Section
  ...
  - [ ] 6.2 Log applied damage and retain the net-wounds tooltip (dep: unified-event-log)
  ...
  - [ ] 6.3 Write render tests for the advanced-qualities collapse
...
- [ ] 7. Add initiative rolling UI (Tier 1, Req 4)
  - [ ] 7.1 Add Roll Initiative controls to `InitiativeTracker.tsx`
  ...
  - [ ] 7.2 Add the initiative-formula setting to `SettingsPage.tsx`
  ...
  - [ ] 7.3 Write tests for initiative UI
...
- [ ] 8. Checkpoint - Tier 1 combat
  - Run combat/initiative tests and type-check; ensure all pass; ask the user if questions arise.

- [ ] 9. New-character onboarding card (Tier 1, Req 5)
  - [ ] 9.1 Implement `GettingStartedCard.tsx` and mount it in `CharacterPage.tsx`
  ...
  - [ ] 9.2 Write render tests for the onboarding card
...
- [ ] 10. PWA install affordance (Tier 1, Req 6)
  - [ ] 10.1 Implement `useInstallPrompt.ts`
  ...
  - [ ] 10.2 Add `InstallPromptControl.tsx` to Settings + one-time hint
  ...
  - [ ] 10.3 Write tests for install affordance
...
- [ ] 11. Advancement completion checklist (Tier 2, Req 7)
  - [ ] 11.1 Implement `AdvancementChecklist.tsx` and mount near the top of `AdvancementPage.tsx`
  ...
  - [ ] 11.2 Write render tests for the checklist
...
- [ ] 12. Take-Damage → critical-wound hand-off (Tier 2, Req 8)
  - [ ] 12.1 Add a critical hand-off control to `TakeDamagePanel.tsx`
  ...
  - [ ] 12.2 Write tests for the hand-off
    - Control appears only at ≤0 wounds / critical state and invokes `onRollCritical`; no new critical logic is introduced
    - _Requirements: 8.1, 8.2, 8.3, 8.4_

- [ ] 13. Combat mode surfacing (Tier 2, Req 9)
  - [ ] 13.1 Improve desktop combat layout and preserve mode state
  ...
  - [ ] 13.2 Write tests for mode surfacing
...
- [ ] 14. Chip controls for combat values (Tier 2, Req 10)
  - [ ] 14.1 Replace the Difficulty and Hit-Location selects with chip controls
  ...
  - [ ] 14.2 Write tests for chip controls (incl. mobile)
...
- [ ] 15. Active house-rule indicators (Tier 2, Req 11)
  - [ ] 15.1 Implement `HouseRuleIndicator.tsx` and render on combat surfaces
...
- [ ] 17. Actionable empty states (Tier 3, Req 12)
  ...
  - [ ] 17.2 Write tests for empty-state copy/actions
    - AttackFlow empty state references adding a weapon; at least one other picker-backed empty state references its add action
    - _Requirements: 12.1, 12.2, 12.3_

- [ ] 18. History review surface (Tier 3, Req 13) (dep: unified-event-log)
  - [ ] 18.1 Surface `TimelineView` with roll+combat filters
  ...
  - [ ] 18.2 Write tests for the history surface
    - Renders events from `eventLog`; filter to `roll` and `combat` works; no separate store introduced
    - _Requirements: 13.1, 13.2, 13.3, 13.4_

- [ ] 19. Keyboard-shortcut discoverability (Tier 3, Req 14)
  - [ ] 19.1 Implement `ShortcutsHelp.tsx` and expose it via the command palette
  ...
  - [ ] 19.2 Write tests for the shortcut list
    - Lists the actually-handled shortcuts; reachable without prior knowledge of a hidden combination
    - _Requirements: 14.1, 14.2, 14.3_

- [ ] 20. Character switcher consolidation (Tier 3, Req 15)
  - [ ] 20.1 Distinguish quick-switch from full management
  ...
  - [ ] 20.2 Write tests for switcher clarity
...
- [ ] 21. Navigation label clarity (Tier 3, Req 16)
  - [ ] 21.1 Clarify the estate section label and sub-tab discoverability
  ...
  - [ ] 21.2 Write tests for nav labels
